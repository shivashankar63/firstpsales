...
import { Loader, Calendar, Clock, User, Building2, Mail, Phone, Briefcase, Eye, MessageCircle, StickyNote, ChevronDown, Phone as PhoneIcon } from "lucide-react";
import DashboardSidebar from "@/components/dashboard/DashboardSidebar";
import { Card } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { getCurrentUser, getLeads, getUserRole, getUsers, getProjects, updateLead, createLeadActivity } from "@/lib/supabase";
...
const SalesFollowUps = () => {
  ...
  const handleSubmitNote = async () => {
    ...
    try {
      ...
      const user = await getCurrentUser();
      if (user) {
        const leadsRes = await getLeads({ assignedTo: user.id });
        const allLeads = leadsRes.data || [];
        const leadsWithFollowUps = allLeads.filter((lead: any) => 
          (lead.next_followup_date || lead.followup_notes) && 
          !['not_interested', 'closed_won'].includes(lead.status)
      ...
      <main className="flex-1 p-2 sm:p-3 lg:p-4 pt-16 sm:pt-16 lg:pt-8 overflow-auto bg-slate-50">
        <div className="mb-2">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-2 gap-2">
            <div>
              <h1 className="text-xl sm:text-2xl font-bold text-slate-900 mb-0.5">My Follow-ups</h1>
            ...
            <div className="flex gap-2">
              ...
              <Button
                onClick={() => navigate('/sales/my-leads')}
                variant="outline"
              >
                Back to Leads
              </Button>
                              ...
                              phoneNumbers.length === 1 ? (
                                ...
                                <DropdownMenu>
                                  <DropdownMenuTrigger asChild>
                                    <Button variant="outline" size="sm" className="h-6 w-6 p-0 hover:bg-slate-100">
                                      <Phone className="w-3 h-3" />
                        ...
                        <TableCell className="py-1 px-2">
                          <div className="flex items-center gap-0.5">
                            <Button
                              variant="outline"
                              size="sm"
                              className="h-6 px-1.5 hover:bg-slate-100 text-[10px]"
                              title="Add Note"
                            ...
                            <Button
                              ...
                              className="h-6 w-6 p-0 hover:bg-slate-100"
                              title="View Details"
                              onClick={() => navigate(`/sales/my-leads?leadId=${followUp.id}`)}
                            >
